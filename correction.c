...
char *dayname_str (dayname day){
    static char* names[]={"MON","THU","WED","THU","FRI","SAT","SUN"};//le static signifie que la variable existe tout au long du programme. Elle existe donc indépendament de la fonction.
    return names[(day-1)%7];//le modulo 7 ne sert à rien
}

char * monthname_str (monthname month){
    static char* names[]={"JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP","OCT","NOV","DEC"};
...
date from_time (time_t when){
    static unsigned int month_len []={31,28,31,30,31,30,31,31,30,31,30,31};
    unsigned long days = when/(60*60*24);
    int daycount;
...
int main (void){
    ...
    printf("%s %s %u %u\n",now.weekday, now.month, now.day, now.year);
